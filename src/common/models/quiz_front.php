...
class QuizModelQuiz extends JModelItem {
	 ...
	 * @return {Array=>Question}
	 */
	public function getQuestionByUnit($unitId) {
		$db = &JFactory::getDBO();
	...
	public function getHistoryListByUid($user_id) {
		$db = &JFactory::getDBO();
		$query = 'SELECT * FROM #__quiz_history WHERE user_id='.$user_id; 
		$db->setQuery($query); 
		$result = $db->loadObjectList();
		return $result;
	}
	...
	public function getHistoryById($hid) {
		$db = &JFactory::getDBO();
		$query = 'SELECT * FROM #__quiz_history WHERE h_id='.$hid; 
		$db->setQuery($query); 
		$result = $db->loadObjectList();
		return $result[0];
	}

	public function addHistory($user_id, $qa_list, $score) {
		$db = &JFactory::getDBO();
		$time = time();
		$query = 'INSERT INTO `#__quiz_history` (`user_id`,`qa_list`,`score`,`created`) 
				  VALUES (\''.$user_id.'\',\''.$qa_list.'\',\''.$score.'\',\''.$time.'\')';
		$db->setQuery($query);
		try {
			$result = $db->query();
			return $result;
	...
	public function deleteHistoryById($hid) {
		$db = &JFactory::getDBO();
		$query = 'DELETE FROM #__quiz_history WHERE h_id='.$hid;
		$db->setQuery($query);
		try {
			$result = $db->query();
			return $result;
